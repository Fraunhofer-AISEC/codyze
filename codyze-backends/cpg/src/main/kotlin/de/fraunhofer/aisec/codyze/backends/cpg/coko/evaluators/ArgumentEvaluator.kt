...
class ArgumentEvaluator(val targetCall: Op, val argPos: Int, val originCall: Op) : Evaluator {
    override fun evaluate(context: EvaluationContext): List<CpgFinding> {
        ...
        for (call in targetCalls) {
            val arg: VariableDeclaration? = (call.arguments.getOrNull(argPos) as? Reference)?.refersTo as? VariableDeclaration
            if (arg in variables) {
                findings.add(
                    CpgFinding(
                        message = "Complies with rule: arg $argPos of \"${call.code}\" stems from a call to \"$originCall\"",
                ...
                findings.add(
                    CpgFinding(
                        message = "Violation against rule: arg $argPos of \"${call.code}\" does not stem from a call to \"$originCall\"",
