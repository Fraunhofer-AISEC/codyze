...
class TSFIInformationExtractor : InformationExtractor() {
    ...
    override fun extractInformation(result: TranslationResult) {
        ...
        for (tsfi in tsfis) {
            ...
            val tsfiDeclaration = TSFI(
                description,
                behavior ?: Name(""),
                annotationExtendedNode,
                tsfiSFs.flatMap { tsfiSFName ->
                    securityFunctionMap.entries.filter {
                        it.key.toString().substringAfterLast(".") == tsfiSFName.toString().substringAfterLast(".")
                    }.map { it.value }
                }.toMutableSet()
            )
        ...
        logger.info {
            "TSFI: ${tsfiDeclarations.size}: $preSF security functions explicitly specified, $postSF security functions identified through analysis."
        }
    }
    ...
    private fun replaceSFName(name: String): String {
        var sfname = name.substringAfterLast(
            "de.fraunhofer.aisec.codyze."
        ).replace(".TOEDefinitions.SecurityFunction", "")
    ...
    data class TSFI(
        ...
        val securityBehavior: Name,
        val functions: Set<Node>,
        val sf: MutableSet<SecurityFunction>
    )
