...
class OrderEvaluator(val baseNodes: Collection<Node>?, val order: Order) : Evaluator {
    private fun findInstancesForEntities(rule: KFunction<*>): Collection<Node> = TODO("Use Codyze-v2 algorithm to find VariableDeclarations for Entities automatically")

    /**
     * Filter the [OrderNode] by the given type. This also works for nested [OrderNodes]s using depth first search (DFS)
     */
    ...
    override fun evaluate(context: EvaluationContext): EvaluationResult {
        ...
        val usedBases = syntaxTree.filterIsInstanceToList<TerminalOrderNode>().map { it.baseName }.toSet()
        if (usedBases.size > 1) {
        ...
        val opsInConcept = implementation::class.declaredMemberFunctions.filter { it.returnType in listOf(FunctionOp::class.createType(), ConstructorOp::class.createType()) }

        val nodesToOp = opsInConcept.flatMap {
            ((it.call(implementation)) as Op).getAllNodes().filterIsInstance<Node>().map { node -> node to it.name }
        }.toMap().toMutableMap()

        nodesToOp.putAll(order.userDefinedOps.entries.flatMap { (opName, op) -> op.invoke().getNodes().filterIsInstance<Node>().map { it to opName } }.toMap())
        ...
        for (node in orderStartNodes) {  // TODO: orderStartNodes should be variable declarations!
            ...
            isOrderValid = isOrderValid && dfaEvaluator.evaluateOrder(dfa, node.followPrevEOG { it.start is Declaration }!!.last().start)
        }
