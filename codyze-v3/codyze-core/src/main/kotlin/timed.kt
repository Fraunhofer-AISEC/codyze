 ...
 * @param message The message to log. The resulting log has the format: '{message} {time the task took in milliseconds} ms.'
...
inline fun <reified T> timed(message: String? = null, block: () -> T): T {
    ...
    log.debug {
        message + " ${ blockDuration.inWholeMilliseconds } ms."
    }
    return result
}
