...
class DFA(states: Set<State> = setOf()) : FSM(states) {
     ...
     * This equivalence check is an improvement on the original algorithm developed by Hopcroft and Karp described [here](https://ecommons.cornell.edu/handle/1813/5958)
     ...
     * - FIND-SET(i): returns the identifier Si of the set which contains i. If no set containing i is found, MAKE-SET(i) is called and the set Si = {i} is created.
     * - UNION(i,j): combines the sets identified by i and j in a new set S = Si ∪ Sj; Si and Sj are destroyed.
     *
     * The algorithm:
     ...
     * 5. If 4. was completed without aborting in 4.2, the two DFAs accept the same language and are thus equivalent: return True
     */
    override fun equals(other: Any?): Boolean {
        if (other is FSM) {
            ...
            /**
             * FIND-SET(i): returns the identifier Si of the set which contains i. If no set containing i is found, MAKE(i) is called and the set Si = {i} is created.
             */
            ...
            val otherDfa = when(other) {
                is NFA -> other.toDfa()
                is DFA -> other.deepCopy()  // make sure to create a deep copy to not alter the passed object when changing the state names
            }
            ...
            check(p0 != null && q0 != null){ "In order to compare to FSMs, both must have exactly one start state." }
            ...
            while (statesToExplore.size > 0) {
                ...
                val allPossibleEdges = setOf(p1,q1).flatMap { it.outgoingEdges }  // get all possible transitions for both states
                for (edge in allPossibleEdges) {  // for a∈Σ
                    ...
                    if (pPrime == null || qPrime == null) return false  // this is because [findSet] cannot handle null as input
