...
class NFA(states: Set<State> = setOf()) : FSM(states) {
    ...
    override fun addState(isStart: Boolean, isAcceptingState: Boolean): State {
        val newState =
            NfaState(name = states.size, isStart = isStart, isAcceptingState = isAcceptingState)
        addState(newState)
        return newState
    }
     ...
     * [powerset construction](https://en.wikipedia.org/wiki/Powerset_construction) algorithm to
     * convert it to a [DFA]
