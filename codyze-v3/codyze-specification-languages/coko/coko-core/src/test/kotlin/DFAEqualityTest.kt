...
/**
 * Tests whether a the [DFA.equals] method works as expected and correctly identifies whether two DFAs accept the same language.
 * If this test fails, the [NFAConstructionTest] suite and [DFAConstructionTest] suite will also fail.
 */
class DFAEqualityTest {
    ...
    fun `test isomorphic DFA`() {
        val getStates = { offset: Int ->
            ...
            val state1 = DfaState(1+offset, isStart = true).apply {
    ...
    @Test
    /** Tests a DFA and an equivalent minimal DFA. Uses the two DFAs depicted [here](https://en.wikipedia.org/wiki/DFA_minimization) */
    fun `test bloated and minimal DFA`() {
        ...
        val state33 = DfaState(3).apply {
            addEdge(Edge("1", nextState = this))
            addEdge(Edge("0", nextState = this))
        }  // a,b
        val state22 = DfaState(2, isAcceptingState = true).apply {
            addEdge(Edge("0", nextState = this))
            addEdge(Edge("1", nextState = state33))
        }  // c,d,e
        val state11 = DfaState(1, isStart = true).apply {
            addEdge(Edge("0", nextState = this))
    ...
    fun `test non equivalent DFA`() {
        ...
        val state33 = DfaState(3).apply {
            addEdge(Edge("1", nextState = this))  // this is where an equivalent DFA would need an additional Edge("0", nextState=this)
        }  // a,b
        val state22 = DfaState(2, isAcceptingState = true).apply {
            addEdge(Edge("0", nextState = this))
            addEdge(Edge("1", nextState = state33))
    ...
    fun `test DFA with a single difference`() {
        // construct another DFA that does not accept the same language
        val state3 = DfaState(3).apply {
            addEdge(Edge("1", nextState = this))
        }  // a,b
        val state2 = DfaState(2, isAcceptingState = true).apply {
            addEdge(Edge("0", nextState = this))
            addEdge(Edge("1", nextState = state3))
        }  // c,d,e
        val state1 = DfaState(1, isStart = true).apply {
            addEdge(Edge("0", nextState = this))
