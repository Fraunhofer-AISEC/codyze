...
import de.fraunhofer.aisec.codyze.mark.analysis.*;
import de.fraunhofer.aisec.codyze.mark.analysis.GraphInstanceContext;
...
import de.fraunhofer.aisec.codyze.mark.markmodel.MOp;
import de.fraunhofer.aisec.codyze.mark.markmodel.MRule;
import de.fraunhofer.aisec.codyze.mark.markmodel.Mark;
...
import static de.fraunhofer.aisec.codyze.mark.analysis.markevaluation.EvaluationHelperKt.*;
...
public class Evaluator {
	...
	private Collection<Finding> getFindings(@NonNull Map<Integer, MarkIntermediateResult> result, @NonNull MarkContextHolder markCtxHolder,
		...
		for (Map.Entry<Integer, MarkIntermediateResult> entry : result.entrySet()) {
			...
			if (evaluationResultUb instanceof Boolean) {
				...
				ResultKind kind = Boolean.TRUE.equals(evaluationResultUb) ? ResultKind.Pass : ResultKind.Fail;
				...
				if (!c.isFindingAlreadyAdded()) {
					...
					Finding f = createFinding(evalResult, c, rule, ResultKind.Open);
					findings.add(f);
				}
			} else {
				log.error(
	...
	private Finding createFinding(ConstantValue evalResult, MarkContext c, MRule rule, ResultKind kind) {
		/*
		 * if we did not add a finding during expression evaluation (e.g., as it is the case in the order evaluation), add a new finding which references all
		 * responsible vertices.
		 */
		...
		return new Finding(
			...
			"Rule "
					+ rule.getName()
					+ (kind != ResultKind.Pass ? " violated" : " verified"),
			currentFile,
			ranges,
			kind);
	}
