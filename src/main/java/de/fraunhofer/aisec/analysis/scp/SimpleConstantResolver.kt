...
class SimpleConstantResolver : ConstantResolver {
	 ...
	 */
	override fun resolveConstantValues(declRefExpr: DeclaredReferenceExpression): Set<ConstantValue> {
		...
		val `val` = resolveConstantValueOfFunctionArgument(declRefExpr.refersTo as? VariableDeclaration, declRefExpr)
							...
							if (lastExpressionInList is Literal<*>) {
								...
								if (refersTo.hasNext()) {
									...
									if (v is VariableDeclaration) {
										...
										val constantValue =
											resolver.resolveConstantValueOfFunctionArgument(v, lastExpressionInList)
										if (constantValue.isPresent) {
											return constantValue
										}
			...
			if (it is Literal<*>) {
				...
				if (args.size == 1) {
					...
					if (init is Literal<*>) {
						...
						log.warn("Cannot evaluate ConstructExpression, it is a {}", init.javaClass.simpleName)
					}
				...
				if (initializers.hasNext()) {
					...
					if (init is Literal<*>) {
						...
						log.warn("Cannot evaluate initializer, it is a {}", init.javaClass.simpleName)
					}
				} else {
