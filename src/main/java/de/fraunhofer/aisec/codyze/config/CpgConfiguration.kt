...
class CpgConfiguration {
    ...
    @JsonIgnore
    @Option(names = ["--enable-python-support"], description = ["Enables the experimental Python support. Additional files need to be placed in certain locations. Please follow the CPG README."])
    var enablePython = false

    @JsonIgnore
    @Option(names = ["--enable-go-support"], description = ["Enables the experimental Go support. Additional files need to be placed in certain locations. Please follow the CPG README."])
    var enableGo = false
...
class TranslationSettings {
    @Option(names = ["--analyze-includes"], description = ["Enables parsing of include files. By default, if --includes are given, the parser will resolve symbols/templates from these include, but not load their parse tree. This will enforced to true, if unity builds are used."], fallbackValue = "true")
