...
public class ExpressionEvaluator {
	...
	private ResultWithContext evaluateBuiltin(FunctionCallExpression expr) {
		...
		for (Builtin b : BuiltinRegistry.getInstance().getRegisteredBuiltins()) {
			if (b.getName().equals(functionName)) {
				return b.execute(expr.getArgs(), this);
			}
		}
	...
	private ResultWithContext evaluateOperand(Operand operand) {
		MarkVariableAssignment markVariableAssignment = variableContext.get(operand.getOperand());
		if (markVariableAssignment == null) {
			log.error("{} does not have a value", operand.getOperand());
			return null;
		}
		ResultWithContext result = ResultWithContext.fromLiteralOrOperand(markVariableAssignment.value);
		result.setVertex(markVariableAssignment.argumentVertex);
		return result;
	}
